(ns cljs.core.logic.nominal
  ...
  (:require-macros [cljs.core.logic.macros :as l
                    :refer [umi uai llist composeg* bind* mplus* -inc != ==
                            conde fresh run run* run-db run-db* run-nc
...
(extend-protocol INomSwap
  ...
  l/LCons
  (swap-noms [t swap s]
    (let [[tfirst s] (swap-noms (l/-lfirst t) swap s)
          [tnext s] (swap-noms (l/-lnext t) swap s)]
...
(deftype Nom [lvar]
  ...
  (-pr-writer [x writer opts]
    (-write writer (str "<nom:" (.-name lvar) ">"))))
...
(defn- -do-suspc [t1 t2 swap a]
  (let [x (loop [^not-native vs #{t2} ^not-native seen #{}]
